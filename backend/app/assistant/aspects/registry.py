"""Static metadata registry for ``RequiredAspect`` semantics.

This module is the single source of truth for aspect metadata consumed
by the answerability judge, web fallback query construction, safety
threshold selection, and diagnostics. The metadata is expressed as
pure Python literals with frozen dataclasses; deterministic keyword
matching MUST NOT decide whether evidence covers an aspect (that is
the answerability judge's role).
"""
...
__all__ = ["REQUIRED_ASPECT_METADATA", "RequiredAspectMetadata"]
