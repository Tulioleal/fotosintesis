...
import { taxonomyCandidateSchema } from "@/lib/api/generated-contracts";
...
export async function POST(request: Request) {
  ...
  if (response.ok) {
    const parsed = taxonomyCandidateSchema.safeParse(payload);
    if (!parsed.success) {
  ...
  return NextResponse.json(
    typeof payload === "object" && payload !== null
      ? payload
      : { detail: "Unable to create manual candidate" },
