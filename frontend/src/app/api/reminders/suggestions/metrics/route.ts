...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${API_BASE_URL}/reminders/suggestions/metrics`, {
    method: "POST",
    headers: { ...authHeaders, "Content-Type": "application/json" },
    body: await request.text(),
    cache: "no-store",
  });
  const payload = await response.json().catch(() => ({ detail: "Unable to record suggestion metric" }));
