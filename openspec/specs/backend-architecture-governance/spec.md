...
Define the backend architecture governance rules enforced after the `backend-dead-code-architecture-refactor` change: verified dead-code removal, behavior-preserving module decomposition, provider/assistant layer independence, assistant graph topology preservation, test-suite decomposition, and automated layering and file-size enforcement.
