...
The assistant SHALL use context-aware validation thresholds when evaluating evidence against required aspects. The threshold selection SHALL depend on the aspect's safety sensitivity and the structural strength of the judge result.
...
- **WHEN** the requested aspect is `pet_toxicity` or `human_edibility`
