...
- **THEN** it creates a bootstrap state bucket and separate dev/prod environment state buckets with versioning, uniform bucket-level access, configurable names, and documented backend prefixes

#### Scenario: Bootstrap migrates its own state
- **WHEN** the first bootstrap apply has created the bootstrap state bucket
- **THEN** the operator can migrate bootstrap state from local state to the bootstrap GCS backend using documented backend configuration without recreating foundation resources
...
#### Scenario: Bootstrap owns GitHub OIDC and CI/deploy identities
- **WHEN** GitHub Actions authenticate to GCP
- **THEN** per-project Workload Identity pools/providers, CI service accounts, deploy service accounts, CI/deploy project IAM, and GitHub principalSet impersonation bindings are owned by bootstrap outputs and not by dev/prod environment roots
...
- **WHEN** dev/prod environment roots are inspected
- **THEN** they do not call project-service enablement modules, create GitHub Workload Identity providers, create CI/deploy service accounts, bind GitHub principalSets to service accounts, create remote state buckets, or output bootstrap-owned values
...
The system SHALL explicitly grant the destination-environment CI service account cross-project read access to the source-environment Artifact Registry so a single OIDC-authenticated token can both verify source images and copy them into the destination registry.
...
#### Scenario: Release workflow authenticates once for both registries
- **WHEN** `release.yml` `promote-images` runs
- **THEN** a single OIDC-authenticated access token is reused to log in to both the dev source registry and the prod destination registry, because the dev project has explicitly granted the prod CI service account the reader role
...
- **WHEN** operators configure GitHub repository and environment variables
- **THEN** remote state bucket names, Workload Identity provider IDs, CI service account emails, deploy service account emails, project IDs, project numbers, and bootstrap recovery values are read from bootstrap outputs rather than dev/prod environment outputs
